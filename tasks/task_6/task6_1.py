...
if __name__ == "__main__":
    ...
    with screen.container():
        ...
        logger.info("1. Initialize the DocumentProcessor")
        doc_processor = DocumentProcessor()

        logger.info("2. Ingest documents")
        doc_processor.ingest_documents()

        # 2) Initalize the EmbeddingClient from Task 4 with embed config
        logger.info("3. Initialize the EmbeddingClient")
        embed_client = EmbeddingClient(**embed_config)

        # 3) Initialize the ChromaCollectionCreator from Task 5
        logger.info("4. Initialize the ChromaCollectionCreator")
        ...
        with st.form("Load Data to Chroma"):
            ...
            if submitted:
                logger.info("5. Checking for uploaded files and topic input")
                ####### YOUR CODE HERE #######
                
                # 5) Use the create_chroma_collection() method to create a Chroma collection from the processed documents

                try:
                    logger.info("6. Creating Chroma collection.")
                    chroma_collection = chroma_creator.create_chroma_collection()
                        
                    logger.info(f"7. Querying Chroma collection with topic: {topic_input}")
                    ...
                    logger.error(f"10. An error occurred: {str(e)}")
                    st.error("11. An error occurred during document processing or collection creation.")
                
            else:
                logger.warning("13. Missing files or topic input.")
                st.error("14. Please upload documents and enter a quiz topic.")
                ####### YOUR CODE HERE #######
